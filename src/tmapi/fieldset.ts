...
export class Fieldset extends EventEmitter {
  ...
  private connectArgs!: Fieldset.Connect;
  private matchData: Fieldset.MatchStats = {
    secRem: 0,
    fieldId: 0,
    state: 'DISABLED',
    match: '',
  ...
  async syncOBS(fieldId: number | undefined | null) {
  ...
  async setDisplay(
    display: Fieldset.Displays,
    displayOptions?: Fieldset.DisplayOptions
  ) {
    this.ws.send(
      JSON.stringify({
        action: 'setScreen',
  ...
  async connectOBS(args: OBS.Connect) {
  ...
  constructor(args: Fieldset.Connect) {
    super();
    this.connect(args);
        ...
        case 'matchStopped':
          ...
          if (this.matchData.secRem === 0 || this.matchData.secRem === 1) {
            this.timeout[0] = setTimeout(() => {
              this.setDisplay(Fieldset.Displays.SavedMatchResults);
            }, 5000);
            this.timeout[1] = setTimeout(() => {
              this.queueNextMatch();
            }, 8000);
            this.timeout[2] = setTimeout(() => {
              this.setDisplay(Fieldset.Displays.InMatch);
            }, 12500);
          }
